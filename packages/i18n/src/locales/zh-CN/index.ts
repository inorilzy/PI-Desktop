...
export const zhCN = {
  ...
  chat: {
    ...
    placeholderHint: "输入 / 使用命令 · @ 引用文件或会话",
    placeholderHomeHint: "输入 / 使用命令 · @ 引用文件或会话",
    ...
    fileGroup: "文件",
    sessionGroup: "会话",
    sessionReferenceChanged: "加载引用时，目标会话、模型或运行轮次已变更。草稿尚未发送，请重试。",
    sessionReferenceBudgetBlocked: "最新的完整轮次超出了引用预算。请在「设置 > AI」中提高「会话引用预算」、腾出上下文空间或减少引用会话。草稿尚未发送。",
    sessionReferenceIncomplete: "未能在引用读取限制内获取最新的完整轮次。请在「设置 > AI」中提高「会话引用预算」或减少引用会话后重试。草稿尚未发送。",
    sessionReferenceSummary: "会话引用：{{turns}} 个完整轮次，约 {{tokens}} 个 token。",
    sessionReferenceOmitted: "已省略 {{turns}} 个已知的较早轮次。",
    sessionReferenceUnread: "还有历史未读取，省略的总轮数未知。",
    ...
    annotationCommentTitle: "评论",
    annotationCommentPlaceholder: "添加评论",
    annotationEdit: "编辑评论",
...
sklm: {
    ...
    sessionReferenceBudget: "会话引用预算",
    sessionReferenceBudgetDesc: "所有引用的会话共享可用模型上下文中的这份估算预算。保留最新的完整轮次，超出预算的历史会省略并提示，不设固定轮数。Token 数为近似估算，最终仍受运行时上下文限制约束。",
    sessionReferenceBudgetOption: "可用上下文的 {{percent}}%",
    sessionReferenceBudgetDefault: "可用上下文的 {{percent}}%（默认）",
