...
全局桥句柄是那个并不存在的边界：`contextBridge` 把 `window.piDesktop` 定义为不可
配置的自有属性，应用无法删除它，之后加载的模块因此可以触及宿主的整个 preload
...
### 6.2.1 面向自动化运行的测试专用自动同意

自动化运行无法点击原生对话框，因此已经拥有临时数据目录的套件通过在其中创建
一个标记文件来显式选择加入：`<dataDir>/e2e-auto-consent`，其中 `<dataDir>`
是该次运行所用的目录（`PI_DESKTOP_DATA_DIR`，常量名见
`apps/desktop/electron/main/tool-permission-consent.ts` 中的 `AUTO_CONSENT_MARKER_FILE`）。

当该标记存在**且**构建未打包时，工具权限确认（spec 03-runtime/01-ipc-protocol §10）
会自己把请求回答为**允许一次**。它的做法是产生 "Allow once" 按钮索引所映射的那个决定，
走的是与一次点击完全相同的映射和相同的下游权限流程：这个开关既不绕过弹窗词表，
也不绕过授权本身。因此它授予不了单次点击授予不了的东西，而且每次自动回答都会写下
一行审计，写明它批准的工具。

两条性质把这个能力与真实用户隔开：

- 当 `app.isPackaged` 为真时它被忽略，即便标记文件就在那里：发布版本里根本不存在
  自动同意路径。
- 没有标记文件的运行只做一次文件存在性检查，然后和以前完全一样显示原生弹窗。
