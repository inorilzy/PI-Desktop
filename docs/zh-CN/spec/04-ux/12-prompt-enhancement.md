...
改写使用哪个模型、用多大思考强度，属于模型层面的决定，位于设置 → 模型 中一张独立
卡片「增强提示词」内，排在「默认项」卡片之后。该卡之所以自带标题，是因为它包含
一个模型选择器——与默认模型行一样，需要标题、当前值与控件，无法套用「默认项」卡那种
单选控件的行形态。
...
该卡片的模型行标题为「默认模型」，使用与「默认项」卡片相同行的锚定可搜索菜单，因此该页只呈现一种模型选择交互。
