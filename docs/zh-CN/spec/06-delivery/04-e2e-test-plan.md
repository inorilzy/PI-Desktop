...
- **步骤**：1) 勾选一个模型的自动调度许可，保存并重新打开，重启后确认仍保留。
  2) 检查父代理模型摘要与 Task 定义目录。3) 对未固定模型的定义显式选择已启用的模型。
  ...
  10) 为固定模型的定义传入其他已启用模型、父模型、按需键或未知键。
  11) 传入冲突的 AI 选择，并分别验证主模型成功、前一至三个模型失败、整个 fallback 链失败。
- **预期**：自动调度许可持久化。系统模型摘要只列出成功解析且已勾选的模型，不包含
  仅供定义固定使用的模型；Task 定义目录展示默认模型并提示省略或重复该键保留默认。
  新委托以定义固定模型为准，忽略其他 Task.model（包括父模型回显），不解析该覆盖键，
  并在返回中说明实际使用的主模型。仅未固定模型时按 Task.model → 会话模型选择；
  未固定定义的未配置或未授权选择返回工具错误，不发送子模型请求。固定模型无需自动调度许可。
  提供商请求严格遵循主模型 → 定义 fallback 顺序，成功即停止，全部失败后不得转用 AI 模型池或父模型。
  主模型配置缺失仍在启动前报错。当前会话模型的继承例外仅适用于未固定定义，按需解析仍需 main 检查许可，且不得覆盖固定模型或替换空闲运行时。许可变化会在下一
...
- **状态**：部分自动化。`pnpm test:e2e:subagent-models` 通过真实 NDJSON 驱动构建后
  的 sidecar，并使用本地确定性 SSE 模型夹具，覆盖跨定义拒绝、自身固定模型回显、固定模型优先于 AI 选择、
  冲突 AI 选择下的有序 fallback、未固定定义选模、按需授权且不重建运行时、会话继承与跨回合撤销。运行时单测覆盖选择校验，桌面启动测试
