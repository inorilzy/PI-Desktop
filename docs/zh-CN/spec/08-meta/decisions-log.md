...
| D442 | Composer `@` 引用其他会话 | **修订 ADR 0024 / ADR 0070 / ADR 0163：`@` 补全在 Files 之上列出 Sessions。选中后插入标题芯片，序列化为 `@session:<uuid>`。发送时通过现有 `session.get` 分页冻结预算选择的完整问答，默认占用剩余可用上下文的 25%，可在设置 > AI 调整。最新一轮必须放下，否则拦发送并保留草稿。见 ADR 0276 与 E2E-CHAT-composer-session-mention。** | 用户需要像 `@` 文件一样指向另一段对话，并让当前模型看到其近期问答，而不恢复已撤回的 A2A。 |
