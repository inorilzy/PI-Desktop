...
| D430 | Composer @ mentions other sessions | **Amend ADR 0024 / ADR 0070 / ADR 0163: the `@` autocomplete lists a Sessions group above Files. Accepting a session inserts an inline title chip that serializes to `@session:<uuid>`, not a filesystem path. Session chips survive workspace switches, are never structured attachments, and open the durable session from the transcript. No host-core, IPC, schema, or Task-family change. See ADR 0266 and E2E-CHAT-composer-session-mention.** | Users needed to address another conversation from the composer the same way they `@` a file, without restoring withdrawn A2A or inlining the other transcript. |
