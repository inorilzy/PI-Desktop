...
| D430 | Composer @ mentions other sessions | **Amend ADR 0024 / ADR 0070 / ADR 0163: the `@` autocomplete lists a Sessions group above Files. Accepting a session inserts an inline title chip that serializes to `@session:<uuid>`, not a filesystem path. Session chips survive workspace switches, are never structured attachments, and open the durable session from the transcript. At send time the desktop follows existing physical-history cursors and freezes a budget-selected suffix of complete Q&A turns, with no fixed turn count. All sources share an estimated context budget (25% of available space by default, adjustable in Settings > AI); newest turns must fit together or sending is blocked without consuming the draft. Whole older turns are added round-robin, with explicit known-omission and unread-history notices. A turn concatenates each independently cleaned, completed parent assistant `content` before the next user, including empty user boundaries (no thinking, tools, nested delegates, or failed/streaming replies). The 400-line page size and visible I/O safety stops do not masquerade as complete-history limits. Frozen queued snapshots are not re-read. No host-core, IPC, schema, or Task-family change. See ADR 0266 and E2E-CHAT-composer-session-mention.** | Users needed to address another conversation from the composer the same way they `@` a file, and to give the current model that conversation's recent Q&A without restoring withdrawn A2A. |
