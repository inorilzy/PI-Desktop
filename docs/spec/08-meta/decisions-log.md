...
- Decision D278 extends D201 / ADR 0062 / ADR 0089 in the agent runtime and
  ...
  (`"provider/modelId"`). Amendment (2026-09-23, ADR subagent-pinned-model-priority):
  a new delegation uses its definition pin first and ignores conflicting
  Task.model choices. Only unpinned definitions select Task.model → session
  model. Failures advance through the definition's configured fallbacks only.
...
- When an unpinned definition's Task.model is not configured or not enabled
  for delegation, the Task tool returns a tool error listing available models. An
