...
/**
 * Serve a trusted plugin's renderer modules over a host-owned scheme.
 *
 ...
 * `manifest.renderer`, and only for files inside that plugin's package. The
 ...
 * may ship data and source maps; nothing here is inert enough to be worth a
 * second allowlist, so everything else is answered with 404 rather than an
 * empty body a loader would have to interpret.
 */
 ...
 * are what a renderer-side `fetch()` of module source needs from a `file://`
 * origin.
 */
  ...
  protocol.handle(PLUGIN_RENDERER_SCHEME, (request) => {
    ...
    return new Response(bytes, {
      ...
      headers: {
        ...
        // The requesting origin is the app's own `file://` document, which is
